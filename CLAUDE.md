...
| `skills/` | `start`, `ship`, `pr-watch`, `refactor`, `audit`, `gain` — each a `SKILL.md`. `ship` owns the full phase ladder (`fresh→scaffolded→building→built→pr-open→merged`), detected from git + GitHub with no state file: re-run `/ship` to resume interrupted work or close out a merged PR, and `/ship pause` to checkpoint mid-flight. `refactor` dispatches `supera-engineer` against existing code (standalone or mid-ship), lightweight — no worktree/PR by default. `audit` is the standalone auditor orchestrator — it runs the security auditor against a branch via its own worktree/PR, decoupled from `/ship`, interactive or `--non-interactive`. `gain` prints the dev's own telemetry dashboard from the local `~/.supera/events.jsonl`. |
| `agents/` | `supera-engineer` (the implementer), `supera-security-auditor`. |
| `hooks/` | `hooks.json` registers the `SessionEnd` hook (`session-end.ts`) that captures a privacy-safe run-metrics event from the local Claude Code session into `~/.supera/events.jsonl` — local-only unless `SUPERA_METRICS=1`. Runs under Node's native TS stripping, so it ships with no consumer build/devDeps. |
| `schema/` | `supera.schema.json` — the per-repo `.claude/supera.json` contract (**source of truth**, update before changing what skills read). `receipt.schema.json` — the `supera-engineer`→`ship`/`pr-watch` JSON receipt. `audit-receipt.schema.json` — the auditor receipt. `metrics-event.schema.json` — the privacy-safe run-telemetry contract (structural no-free-text invariant; `validate.ts` enforces it). |
...
- **One phase ladder, one owner.** The lifecycle (`fresh→scaffolded→building→built→pr-open→merged`) is derived from git + GitHub — **no state file, no tracker**. `/ship` owns the whole ladder and is idempotent: it resumes interrupted builds, opens the PR, and on a merged PR closes out + tears down the worktree. `/ship pause` writes the `wip:` checkpoint. `/pr-watch` drives the open PR green and auto-hands-off merged PRs to `/ship` (which owns close-out + teardown).
