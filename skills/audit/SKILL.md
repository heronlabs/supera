...
**No commits (pure report-only)** → open **no** PR. Render the report from the receipt's `findings[]` + `degraded[]` (`applied[]` is empty here). Tear down the worktree (remove the worktree, delete the branch) and exit.
